...
\begin{algorithm}
	\begin{algorithmic}
		\Procedure{genMicrostructure}{input: microstructure $M_i$, output: microstructure $M_o$}
		\State $M_o\gets M_i$
		\While {some voxels of $M_o$ have not been visited}
		\While {microstructure $M_o$ is unchanged}
		\State pick a random voxel $v$ of $M_o$ that has not been visited %yet
		\State assign a randomly chosen material to $v$
		\If {$M_o$ is manifold and $M_o\neq M_i$} \State{accept the change} \EndIf
		\EndWhile
		\EndWhile
		\EndProcedure
	\end{algorithmic}
...
To force continuous material ratios towards discrete values, we experimented with the SIMP model with the exponent set to $3$ and the Hashin-Shtrikman bound for isotropic materials described by Bends{\o}e and Sigmund \shortcite{bendsoe1999material}.
