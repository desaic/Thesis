...
original topology problem [Allaire 2012].
While very powerful, these methods are limited to the minimum compliance problem, for
136 which the link between stiffness and optimal density can be analyt137
ically formulated. In a sense, our work can been seen as a gener138
alization of this type of approaches to handle arbitrary materials in
broader contexts.
140 While largely used in engineering, standard methods for topol141
ogy optimization suffer from a major drawback. Indeed, the
142 parametrization of the problem at the voxel level makes them ex143
tremely expensive and largely impedes their use on high resolu144
tions models such as the ones generated by modern 3D printing
145 hardware. To counteract the effects of this explosion of variables in
146 finely discretized layouts, Rodrigues et al. [2002] proposed an inter147
esting formulation where microstructure designs and macroscopic
148 layouts using the effective properties of the underlying microstruc149
tures were hierarchically coupled and treated simultaneously. This
150 initial work has been extended in multiple ways [Coelho et al. 2008;
151 Nakshatrala et al. 2013; Yan et al. 2014; Xia and Breitkopf 2014].
152 However, these methods still need to handle variables defined at
153 the microstructure level and therefore they remain relatively costly.
154 The closest to our work is probably the method proposed by Xia
155 et al.[2015], which also relies on a database to speed up compu156
tations. However, their work specifically targets minimum com157
pliance problem in the structural design which allows them to ap158
proximate the macroscale behaviour of the microstructures with a
159 particular strain-based interpolating function.
