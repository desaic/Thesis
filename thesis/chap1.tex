...
Computational simulation tools allow user to tinker a virtual design and quickly preview the predicted physical performance without laboriously 
constructing new prototypes.
This saves time and cost in the design process.
Of course, a simulation algorithm needs to be \textit{predictive} in the sense that simulated results should agree with real-world experiments.
...
On top of being predictive, the simulation should be \textit{efficient} to provide interactive feedback to a designer so that he can quickly try different ideas for design improvement.
Taking a step further, 
a computational design algorithm can automatically tweak design parameters and evaluate the performance by keeping simulation tools in the loop.
With a predictive simulation, a local optimization algorithm can find a local optimum of the design parameters that correspond to real-world improvement of the design.

Our goal in this thesis is to develop predictive and efficient simulation tools to enable computational design optimization of deformable objects.
The gold standard technique for estimating the mechanical behavior of a deformable object under load is the finite element method (FEM).
While accurate, FEM is notoriously slow, making it a major bottleneck in the iterative design process.
...
We propose to improve efficiency of FEM simulation using low-resolution meshes while maintaining accuracy by computing new material models for the coarse finite elements.
...
For dynamic problems, we use gradient-based optimization algorithm to find locally optimal designs.
